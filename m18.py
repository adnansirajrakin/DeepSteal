...
parser.add_argument('--error', type=float, default=0.0001, help='error rate')
...
net_f = RESNET18(num_classes=10, pretrained=False)
...
'''
perc = torch.tensor([0.71,0.032,0.043,0.031,0.042,0.043,0.042])
...
if args.evaluate==0:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        _ = functional(classifier2, adv_classifier,attacker, device, epoch=0)
        if epoch == 111:
            classifier2 = torch.load('4.pt')
        if best_acc < accs:
            best_acc = accs
            torch.save(classifier2, '4.pt')
