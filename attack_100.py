...
from resnet import resnet18,resnet34,Resnext50_32x4d
from resnet2 import RESNET18,RESNET34,resnext50_32x4d
...
parser.add_argument('--batch-size', '-b', type=int, default=128, metavar='N', help='input batch size for training (default: 500)')
...
net_f = resnext50_32x4d(num_classes=100)
...
for name, module in adv_classifier.named_modules():
    if isinstance(module, nn.Linear) or isinstance(module, nn.Conv2d):
        ...
        if cc < args.layer:
            ...
            print(recordf[cc].size()[0]/tot,recordm[cc].size()[0]/tot,record7[cc].size()[0]/tot,record6[cc].size()[0]/tot,record5[cc].size()[0]/tot,record4[cc].size()[0]/tot,record3[cc].size()[0]/tot,record2[cc].size()[0]/tot,record[cc].size()[0]/tot)
            ...
            if isinstance(module1, nn.Linear) or isinstance(module1, nn.Conv2d):
                ...
                if cc==m:
                    if cc < args.layer:
                        ...
                        print(module.weight.size(),module1.weight.size())
...
if args.evaluate==0:
    ...
    for epoch in range(1, args.epochs + 1):
            ...
            if isinstance(module, nn.Linear) or isinstance(module, nn.Conv2d):
                ...
                for name1, module1 in adv_classifier.named_modules():
                    if isinstance(module1, nn.Linear) or isinstance(module1, nn.Conv2d):
                        m+=1
                        if cc==m:
                            print((module.weight.data.view(-1).sign() - module1.weight.data.view(-1).sign()).abs().sum()/2)
        ...
        if epoch == 111:
            classifier2 = torch.load('nm10.pt')
        if best_acc < accs:
            best_acc = accs
            torch.save(classifier2, 'nm10.pt')
